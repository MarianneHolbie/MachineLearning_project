...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        """
            method to Displays the image with all boundary boxes, class names,
             and box scores

            :param image: ndarray, unprocessed image
            :param boxes: ndarray, boundary box for the image
            :param box_classes: ndarray, class indices for each box
            :param box_scores: ndarray, box scores for each image
            :param file_name: file path where the original image is stored

            :return: Displays the image with all boundary boxes, class names,
            and box scores (see example below)
                Boxes : blue line of thickness 2
                Class names and box scores: above each box in red
                    Box scores: rounded to 2 decimal places
                    Text: 5 pixels above the top left corner of the box
                          in FONT_HERSHEY_SIMPLEX (scale 0.5)
                    Line: thickness=1, type LINE_AA
                Window name : file_name
                If s key pressed:
                    Image saved in folder detections in the current directory
                    If detections not exist: create
                    Saved image name: file_name
                    Closed image window
    ...
    def predict(self, folder_path):
        """
            Method to apply prediction on image

            :param folder_path: str, folder holding all images to predict
s
            :return: tuple (predictions, image_paths)
                - predictions : list of tuple for each image (boxes,
                box_classes, box_scores)
                - image_paths: list of image paths corresponding to
        ...
        # pretreat img and load model prediction on this pretreat img
        ...
        for idx in range(len(pimages)):
            ...
            (boxes,
             box_confidences,
             box_class_probs) =\
                (self.process_outputs(output, image_shape[idx]))

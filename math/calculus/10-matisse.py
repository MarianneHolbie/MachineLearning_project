    ...
    """ function : calculate derivative of a polynomial
        ...
        Arguments:
            poly: list of coefficients representing a polynomial
                index : represent the power of x that the coefficient belongs to

        Returns:
            new list of coefficients representing the derivative of the polynomial
    """
    if len(poly) < 0 or not isinstance(poly, list) or not all(isinstance(cuff, (int, float)) for cuff in poly):
        return None
    else:
